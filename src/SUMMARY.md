## User documentation

- [Introduction](./introduction.md)

- [Quick Start](user/tutorials/quickstart.md)


- [User documentation](user/README.md)
  - [Installation](user/installation/installation.md)
    - [Install Epinio cli](user/installation/install_epinio_cli.md)
    - [Install Epinio with custom DNS](user/installation/install_epinio_customDNS.md)
    - [Install Epinio with "magic" DNS](user/installation/install_epinio_magicDNS.md)
    - [Install Epinio on EKS (Amazon)](user/installation/install_epinio_on_eks.md)
    - [Install Epinio on AKS (Azure)](user/installation/install_epinio_on_aks.md)
    - [Install Epinio on GKE (Google)](user/installation/install_epinio_on_gke.md)
    - [Install Epinio on RKE2 (Rancher)](user/installation/install_epinio_on_rke.md)
    - [Install Epinio on k3d (local)](user/installation/install_epinio_on_k3d.md)
    - [Install Epinio on Rancher Desktop (local)](user/installation/install_epinio_on_rancher_desktop.md)
    - [Install Wordpress on Epinio](user/installation/install_wordpress_application.md)
  - [Uninstall Epinio](user/installation/uninstall_epinio.md)

  - [Explanations](user/explanations/explanations.md)

    - [Advanced topics](user/explanations/advanced.md)
    - [Configuration](user/explanations/configuration.md)
    - [Detailed Push Process](user/explanations/detailed-push-process.md)
    - [Principles](user/explanations/principles.md)
    - [Security](user/explanations/security.md)
    - [Windows](user/explanations/windows.md)

  - [HowTos](user/howtos/howtos.md)

    - [Certificate Issuers](user/howtos/certificate_issuers.md)
    - [Provision external IP address for local Kubernetes](user/howtos/provision_external_ip_for_local_kubernetes.md)
    - [Push with gitjob](user/howtos/gitjob_push.md)
    - [Setup external S3 storage](user/howtos/setup-external-s3.md)

  - [Reference documentation](user/references/references.md)

    - [Command requirements](user/references/README.md)
    - [Command reference](user/references/commands.md)
    - [Supported Applications](user/references/supported-apps.md)
